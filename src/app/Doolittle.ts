...
export class Doolittle extends LU {
  private static partialPivotIn(U: Matrix, currentRow: number): number {
    ...
    for (let i = currentRow + 1; i < U.getRows(); i++) if (Math.abs(U.getElement(i, currentRow)) > Math.abs(U.getElement(indexOfMaxPivot, currentRow))) indexOfMaxPivot = i;

    return indexOfMaxPivot;
  }

  public override Decompose(U: Matrix): [Matrix, Matrix, Step[], any[]] {
